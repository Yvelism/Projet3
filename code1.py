...
class perso:
    def __init__(self,choisi, avance, x, y, taille_x=15, taille_y=15, vitesseh=3, vitessev=8):
    ...
    def mouvementh(self):
        if pyxel.bnt(pyxel.MOUSE_BUTTON_LEFT) and self.x<pyxel.mouse_x<(self.x+self.taille_x) and self.y<pyxel.mouse_x<(self.y+self.taille_y)  :
            self.avance = not self.avance
        while self.avance:
            self.x+= self.vitesseh

    #def mouvementv(self):
        #if self.y+self.taille_y< last_floor:
